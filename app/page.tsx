...
function ErrorFallback({ error, resetErrorBoundary }: { error: Error, resetErrorBoundary: () => void }) {
  return (
    <div className="flex items-center justify-center min-h-screen bg-black text-white p-6">
      <div className="max-w-lg w-full bg-gray-900 border border-red-600 shadow-lg rounded p-6 pixel-border">
        <h2 className="text-red-500 text-xl mb-3 pixel-effect" style={{ fontFamily: "var(--font-pixel)" }}>
          Something went wrong
        </h2>
        <pre className="bg-gray-800 p-4 rounded text-sm overflow-auto mb-4 max-h-40 text-red-300">
          {error.message}
        </pre>
...
export default function Home() {
  ...
  useEffect(() => {
    if (!booting && !error) {
      try {
        setContent(
          <main className="flex min-h-screen flex-col items-center justify-between p-5 md:p-12 bg-black bg-noise">
            <Terminal />
